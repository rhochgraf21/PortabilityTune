...
StatusCode GemmTempBufferSize(const Layout layout, const Transpose a_transpose, const Transpose b_transpose,
  ...
  try {
    ...
    /*
    auto input2 = database::Input{0,0,0};
    auto input3 = database::Input{4096,4096,4096};

    auto dbd = db("XgemmDirect");

    
    // FIRST DATABASE FIRST PRINT
    log_debug("Starting looking for 0");
    log_debug("0: " + dbd.GetValuesStringForInput(input2));
    log_debug("Starting looking for 4096");
    log_debug("4096: " + dbd.GetValuesStringForInput(input3));

    // FIRST DATABASE SECOND PRINT
    log_debug("Starting looking for 0");
    log_debug("0: " + dbd.GetValuesStringForInput(input2));
    log_debug("Starting looking for 4096");
    log_debug("4096: " + dbd.GetValuesStringForInput(input3));

    Databases db2(kernel_names);
    Routine::InitDatabase(device, kernel_names, PrecisionValue<T>(), {}, db2);
    auto dbd2 = db("XgemmDirect");

    // SECOND DATABASE FIRST PRINT
    log_debug("0: " + dbd2.GetValuesStringForInput(input2));
    log_debug("4096: " + dbd2.GetValuesStringForInput(input3));

    // SECOND DATABASE SECOND PRINT
    log_debug("0: " + dbd.GetValuesStringForInput(input2));
    log_debug("4096: " + dbd.GetValuesStringForInput(input3));
    */
    auto input = database::Input{m, n, k};

    // TODO, see if GetParameterValue gives correct results here

    auto xgemm_min_indirect_size = db.GetParameterValue(input, "XGEMM_MIN_INDIRECT_SIZE", "GemmRoutine");
    auto mwg = db.GetParameterValue(input, "MWG", "Xgemm");
    auto nwg = db.GetParameterValue(input, "NWG", "Xgemm");
    auto kwg = db.GetParameterValue(input, "KWG", "Xgemm");
    auto kreg = db.GetParameterValue(input, "KREG", "Xgemm");
    auto gemmk = db.GetParameterValue(input, "GEMMK", "Xgemm");
    // auto mwg = db.GetParameterValue("MWG", "Xgemm");

  // Computes the buffer size
    if (Xgemm<T>::UseDirectKernel(m, n, k, xgemm_min_indirect_size)) {
      temp_buffer_size = 0;
    }
    else {
      temp_buffer_size = Xgemm<T>::GetTempSize(layout, a_transpose, b_transpose, m, n, k,
                                               a_offset, a_ld, b_offset, b_ld, c_offset, c_ld,
                                                mwg, nwg, kwg * kreg, gemmk);
    }
