...
R"(#define LOADMATRIXA2 LOADMATRIXA##PARAMS
#define XGEMV2 XGEMV##PARAMS
#define LOADMATRIXA22 LOADMATRIXA2##PARAMS
#define XGEMV22 XGEMV2##PARAMS
#define XGEMV2 XGEMV##PARAMS
#define LOADMATRIXA2 LOADMATRIXA##PARAMS
...
#define LOADMATRIXA2 LOADMATRIXA##PARAMS
#define XGEMV2 XGEMV##PARAMS
#define LOADMATRIXA2 CONCATENATE(LOADMATRIXA,PARAMS)
#define XGEMV2 CONCATENATE(XGEMV,PARAMS)
#define LOADMATRIXA2 CONCATENATE(LOADMATRIXA,PARAMS)
#define XGEMV2 CONCATENATE(XGEMV,PARAMS)
// =================================================================================================
...
INLINE_FUNC real #define LOADMATRIXA LOADMATRIXA##PARAMS
#define LOADMATRIXA LOADMATRIXA##PARAMS
LOADMATRIXA2(const __global real* restrict agm, const int x, const int y,
...
#endif
void #define XGEMV XGEMV##PARAMS
#define XGEMV XGEMV##PARAMS
XGEMV2(const int m, const int n,
  ...
  for (int kwg=0; kwg<n_floor; kwg+=WGS1) {
    ...
    #pragma unroll
    for (int _w = 0; _w < WPT1; _w += 1) {
      const int gid = _w*GET_GLOBAL_SIZE2(0) + GET_GLOBAL_ID2(0);
      if (gid < m) {
        ...
        if (a_rotated == 0) { // Not rotated
          for (int kloop=0; kloop<WGS1; kloop+=UNROLL1) {
            ...
            for (int _kunroll = 0; _kunroll < UNROLL1; _kunroll += 1) {
              ...
              real value = LOADMATRIXA2(agm, gid, k, a_ld, a_offset, parameter, kl, ku);
          ...
          for (int kloop=0; kloop<WGS1; kloop+=UNROLL1) {
            ...
            for (int _kunroll = 0; _kunroll < UNROLL1; _kunroll += 1) {
              const int k = kwg + kloop + _kunroll;
              real value = LOADMATRIXA2(agm, k, gid, a_ld, a_offset, parameter, kl, ku);
              if (do_conjugate == 1) { COMPLEX_CONJUGATE(value); }
              MultiplyAdd(acc1[_w], xlm[kloop + _kunroll], value);
            }
          }
