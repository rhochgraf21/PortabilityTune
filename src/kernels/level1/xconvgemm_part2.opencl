...
R"(#define XCONVGEMMFLIP2 XCONVGEMMFLIP##PARAMS
#define XCONVGEMMNORMAL2 XCONVGEMMNORMAL##PARAMS
#define XCONVGEMMFLIP22 XCONVGEMMFLIP2##PARAMS
#define XCONVGEMMNORMAL22 XCONVGEMMNORMAL2##PARAMS
#define GLOBALTOLOCALCHECKEDIMAGE2 GLOBALTOLOCALCHECKEDIMAGE##PARAMS
#define GLOBALTOPRIVATECHECKEDIMAGE2 GLOBALTOPRIVATECHECKEDIMAGE##PARAMS
#define XCONVGEMMNORMAL2 XCONVGEMMNORMAL##PARAMS
#define XCONVGEMMFLIP2 XCONVGEMMFLIP##PARAMS
...
#define XCONVGEMMFLIP2 XCONVGEMMFLIP##PARAMS
#define XCONVGEMMNORMAL2 XCONVGEMMNORMAL##PARAMS
#define XCONVGEMMFLIP2 CONCATENATE(XCONVGEMMFLIP,PARAMS)
#define XCONVGEMMNORMAL2 CONCATENATE(XCONVGEMMNORMAL,PARAMS)
#define XCONVGEMMFLIP2 CONCATENATE(XCONVGEMMFLIP,PARAMS)
#define XCONVGEMMNORMAL2 CONCATENATE(XCONVGEMMNORMAL,PARAMS)
// =================================================================================================
...
#endif
{
  ...
  if ((idm < (num_patches/WGD)*WGD) && (idn < (num_kernels/WGD)*WGD)) {
    ...
    for (; kwg < (patch_size/WGD) * WGD; kwg += WGD) {
      // Loads data: off-chip --> local (matrix A and B)
        ...
        if (num_patches % VWMD == 0 && col_offset_batch % VWMD == 0) {
          GLOBALTOLOCALDIRECTA2(colgm, alm, num_patches, col_offset_batch, kwg, false, false);
        }
        else {
          GLOBALTOLOCALSCALARA2(colgms, alm, num_patches, col_offset_batch, kwg, false, false);
        }
      #else
        GLOBALTOLOCALCHECKEDIMAGE2(imagegms, alm, image_offset_batch, output_w, kwg,
      ...
      if (patch_size % VWND == 0 && kernel_offset % VWND == 0) {
        GLOBALTOLOCALDIRECTB2(kernelgm, blm, patch_size, kernel_offset, kwg, true, false);
      }
      else {
        GLOBALTOLOCALSCALARB2(kernelgms, blm, patch_size, kernel_offset, kwg, true, false);
      }
      ...
      for (int pwi = 0; pwi < WGD; pwi += KWID) {
        ...
        for (int _pit = 0; _pit < KWID; _pit += 1) {
          ...
          #pragma unroll
          for (int _mi = 0; _mi < MWID; _mi += 1) {
            apd[_mi] = LOCALTOPRIVATEDIRECTA2(alm, _mi, kg, false);
          }
          #pragma unroll
          for (int _ni = 0; _ni < NWID; _ni += 1) {
            bpd[_ni] = LOCALTOPRIVATEDIRECTB2(blm, _ni, kg, true);
          }
    ...
    for (; kwg < patch_size; ++kwg) {
      ...
      for (int _mi = 0; _mi < MWID; _mi += 1) {
        #if defined(CONVGEMM_WITH_IM2COL)
          apd[_mi] = GLOBALTOPRIVATEDIRECTA2(colgms, _mi, num_patches, col_offset_batch, idm, kwg, false, false);
        #else
          const int w_id = (idm + _mi) % output_w;
          const int h_id = (idm + _mi) / output_w;
          apd[_mi] = GLOBALTOPRIVATECHECKEDIMAGE2(imagegms, image_offset_batch, h_id, w_id, kwg,
                                                 input_h, input_w, channels, kernel_h, kernel_w,
                                                 pad_h, pad_w, stride_h, stride_w,
                                                 dilation_h, dilation_w, kernel_flip);
        #endif
      ...
      #pragma unroll
      for (int _ni = 0; _ni < NWID; _ni += 1) {
        bpd[_ni] = GLOBALTOPRIVATEDIRECTB2(kernelgms, _ni, patch_size, kernel_offset, idn, kwg, true, false);
      }
      // Performs the accumulation (Cpmd += Apmd * Bpmd)
      #pragma unroll
      for (int _ni = 0; _ni < NWID; _ni += 1) {
    ...
    for (int _ni = 0; _ni < NWID; _ni += 1) {
      #pragma unroll
      for (int _mi = 0; _mi < MWID; _mi += 1) {
        STORERESULTSDIRECT2(resultgm, cpd[_ni * MWID + _mi], _mi, _ni, idm, idn,
  ...
  // Simple but slower version for the parts on the edge (incomplete tiles in M and N-dimensions)
    ...
    for (; kwg < (patch_size/WGD) * WGD; kwg+=WGD) {
      ...
      #if defined(CONVGEMM_WITH_IM2COL)
        GLOBALTOLOCALCHECKEDA2(colgms, alm, num_patches, col_offset_batch, kwg, false, false, num_patches, patch_size);
      #else
        GLOBALTOLOCALCHECKEDIMAGE2(imagegms, alm, image_offset_batch, output_w, kwg,
                                  input_h, input_w, channels, kernel_h, kernel_w,
                                  pad_h, pad_w, stride_h, stride_w,
                                  dilation_h, dilation_w, kernel_flip);
      #endif
      GLOBALTOLOCALCHECKEDB2(kernelgms, blm, patch_size, kernel_offset, kwg, true, false, num_kernels, patch_size);
      barrier(CLK_LOCAL_MEM_FENCE);
      // Loops over all workitem tiles, unrolled by a factor KWID
      for (int pwi = 0; pwi < WGD; pwi += KWID) {
        #pragma unroll
    ...
    for (; kwg < patch_size; ++kwg) {
      ...
      for (int _mi = 0; _mi < MWID; _mi += 1) {
      #if defined(CONVGEMM_WITH_IM2COL)
        apd[_mi] = GLOBALTOPRIVATECHECKEDA2(colgms, _mi, num_patches, col_offset_batch, idm, kwg, false, false, num_patches);
      #else
        const int w_id = (idm + _mi) % output_w;
        const int h_id = (idm + _mi) / output_w;
        apd[_mi] = GLOBALTOPRIVATECHECKEDIMAGE2(imagegms, image_offset_batch, h_id, w_id, kwg,
      ...
      #pragma unroll
      for (int _ni = 0; _ni < NWID; _ni += 1) {
        bpd[_ni] = GLOBALTOPRIVATECHECKEDB2(kernelgms, _ni, patch_size, kernel_offset, idn, kwg, true, false, num_kernels);
      }
      // Performs the accumulation (C += A * B)
      #pragma unroll
      for (int _ni = 0; _ni < NWID; _ni += 1) {
    ...
    for (int _ni = 0; _ni < NWID; _ni += 1) {
      #pragma unroll
      for (int _mi = 0; _mi < MWID; _mi += 1) {
        STORERESULTSCHECKED2(resultgm, cpd[_ni * MWID + _mi], _mi, _ni, idm, idn, num_patches, num_kernels,
                            ONE, ZERO, num_patches, result_offset_batch, false);
      }
    }
  }
  ...
  __kernel __attribute__((reqd_work_group_size(MDIMCD, NDIMCD, 1)))
#endif
void #define XCONVGEMMFLIP XCONVGEMMFLIP##PARAMS
#define XCONVGEMMFLIP XCONVGEMMFLIP##PARAMS
XCONVGEMMFLIP2(const int num_patches, const int num_kernels, const int patch_size,
                   const __global realND* restrict kernelgm, const int kernel_offset,
                   __global real* resultgm, const int result_offset, const int result_stride,
                   const __global realMD* restrict imagegm, const int image_offset,
                   const int input_h, const int input_w, const int channels,
  ...
  __kernel __attribute__((reqd_work_group_size(MDIMCD, NDIMCD, 1)))
#endif
void #define XCONVGEMMNORMAL XCONVGEMMNORMAL##PARAMS
#define XCONVGEMMNORMAL XCONVGEMMNORMAL##PARAMS
XCONVGEMMNORMAL2(const int num_patches, const int num_kernels, const int patch_size,
                     const __global realND* restrict kernelgm, const int kernel_offset,
                     __global real* resultgm, const int result_offset, const int result_stride,
                     const __global realMD* restrict imagegm, const int image_offset,
                     const int input_h, const int input_w, const int channels,
