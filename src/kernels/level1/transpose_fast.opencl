...
R"(#define TRANSPOSEMATRIXFAST2 TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST22 TRANSPOSEMATRIXFAST2##PARAMS
#define GET_NUM_GROUPS2 GET_NUM_GROUPS##PARAMS
#define TRANSPOSEMATRIXFAST2 TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST2 TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST2 CONCATENATE(TRANSPOSEMATRIXFAST,PARAMS)
#define TRANSPOSEMATRIXFAST2 CONCATENATE(TRANSPOSEMATRIXFAST,PARAMS)
// =================================================================================================
// Data-widths
#if TRA_WPT == 1
...
#endif
void #define TRANSPOSEMATRIXFAST TRANSPOSEMATRIXFAST##PARAMS
#define TRANSPOSEMATRIXFAST TRANSPOSEMATRIXFAST##PARAMS
TRANSPOSEMATRIXFAST2(const int ld,
  ...
  const int gid0 = GET_GROUP_ID2(0);
  #if TRA_SHUFFLE == 1
    const int gid1 = (GET_GROUP_ID2(0) + GET_GROUP_ID2(1)) % GET_NUM_GROUPS2(0);
  #else
    const int gid1 = GET_GROUP_ID2(1);
  #endif
  ...
  for (int _w_one = 0; _w_one < TRA_WPT; _w_one += 1) {
    ...
    const int id_one = gid1 * TRA_DIM + GET_LOCAL_ID2(0);
    const int id_two = (gid0 * TRA_DIM + GET_LOCAL_ID2(1))*TRA_WPT + _w_one;
    // Loads data into the local memory
    realT value = src[id_two*(ld/TRA_WPT) + id_one];
    tile[GET_LOCAL_ID2(0)*TRA_WPT + _w_one][GET_LOCAL_ID2(1)] = value;
  }
  ...
  #pragma unroll
  for (int _w_one = 0; _w_one < TRA_WPT; _w_one += 1) {
    vpm[_w_one] = tile[GET_LOCAL_ID2(1)*TRA_WPT + _w_one][GET_LOCAL_ID2(0)];
  }
  ...
  for (int _w_two = 0; _w_two < TRA_WPT; _w_two += 1) {
    ...
    #endif
    const int id_one = gid0*TRA_DIM + GET_LOCAL_ID2(0);
    const int id_two = (gid1*TRA_DIM + GET_LOCAL_ID2(1))*TRA_WPT + _w_two;
