...
using Input = std::array<size_t, 3>; // input sizes as stored in database (zeroes for unused dimensions)
...
using Entry = std::pair<Input, Parameters>; // input-parameters entries
...
struct DatabaseDevice {
  Name name;
  std::vector<DatabaseInput> inputs;
};
