...
class Routine {
 public:
  ...
  static void InitDatabase(const Device &device, const std::vector<std::string> &kernel_names,
    ...
    for (const auto &kernel_name : kernel_names) {
      ...
      db(kernel_name) = DatabaseCache::Instance().Get(DatabaseKeyRef{platform_id, device(), precision, kernel_name},
                                                       &has_db);
      if (has_db) { 
        log_debug("Database already found"); 
        for(auto &param: db(kernel_name).GetValuesString()) {
          log_debug("Found stored parameter " + param);
        }
        continue; 
      }
      ...
      log_debug("Attempting to store " + kernel_name);
      DatabaseCache::Instance().Store(DatabaseKey{platform_id, device(), precision, kernel_name},
                                      Database{db(kernel_name)});
      log_debug("Stored kernel " + kernel_name);

      
      for(auto &param: db(kernel_name).GetValuesString()) {
        log_debug("Stored parameter " + param);
      }
      // pure test, does the load/store itself work.
      has_db = false;
      db(kernel_name) = DatabaseCache::Instance().Get(DatabaseKeyRef{platform_id, device(), precision, kernel_name},
                                                       &has_db);
      if (has_db) { 
    ...
    log_debug("Finished initializing database");
  }
  ...
  // temporary fallback constructor for testing
  explicit Routine(Queue &queue, EventPointer event, const std::string &name,
 ...
 private:
  ...
  void InitProgram(std::unordered_map<std::string, std::vector<const char *>> source);
