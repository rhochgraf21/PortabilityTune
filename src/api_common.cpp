...
StatusCode RetrieveParameters(const RawDeviceID device, const std::string &kernel_name,
  ...
  try {
    ...
    for (const auto &parameter: database.GetParameters(input)) {
...
StatusCode OverrideParameters(const RawDeviceID device, const std::string &kernel_name,
  ...
  try {
    ...
    const auto database_input = database::DatabaseInput{database::Input{input}, parameter_values};
    const auto database_device = database::DatabaseDevice{database::kDeviceNameDefault, {database_input}};
