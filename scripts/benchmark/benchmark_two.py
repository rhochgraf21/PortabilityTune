...
def run_benchmark(name, arguments_list, precision, num_runs, platform, device, comparisons):
    binary_1 = "./clblast_client_x" + name + "_1" # binary 1: RBlast
    binary_2 = "./clblast_client_x" + name + "_2" # binary 2: CLBlast
    ...
    for arguments in arguments_list:
        ...
        all_arguments.append(" -tuner_files clblast_xgemm_11_32.json")
        
        benchmark_output_2 = utils.run_binary(binary_2, all_arguments)
        result_2 = utils.parse_results(benchmark_output_2)
        
        all_arguments.remove(" -tuner_files clblast_xgemm_11_32.json")
	
        benchmark_output_1 = utils.run_binary(binary_1, all_arguments)
        print("ran")
        result_1 = utils.parse_results(benchmark_output_1)
	...
	# add benchmark two results as comparison to benchmark 1
        pos = 0
        while pos < len(result_2):
                for key in result_2[pos]:
                        if key[-2:] == "_1":
                                result_1[pos][key[:-2]+"_3"] = result_2[pos][key]
                pos += 1
        print(result_1)
        results.extend(result_1)
    return results
...
def benchmark_single(benchmark, comparisons, platform, device, num_runs, precision, load_from_disk,
    ...
    plot.plot_graphs(results["benchmarks"], pdf_file_name, results["num_rows"], results["num_cols"],
                     ...
                     ["Portability Tuned CLBlast (Estimated)", "Tuned CLBlast"], plot_title, tight_plot, verbose)
