...
CLBlastStatusCode PUBLIC_API CLBlastOverrideParameters(const cl_device_id device, const char* kernel_name,
  ...
  try {
    ...
    auto input_ = std::array<size_t, 3>();
    std::copy(input, input+3, std::begin(input_));
    ...
    const auto status = clblast::OverrideParameters(device, kernel_name_cpp, precision_cpp, input_, parameters);
