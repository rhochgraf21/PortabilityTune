...
R"(#define INITACCREGISTERS2 INITACCREGISTERS##PARAMS
...
#define INITACCREGISTERS22 INITACCREGISTERS2##PARAMS
#define GLOBALTOLOCALA22 GLOBALTOLOCALA2##PARAMS
#define GLOBALTOLOCALB22 GLOBALTOLOCALB2##PARAMS
#define GLOBALTOPRIVATEA2 GLOBALTOPRIVATEA##PARAMS
#define GLOBALTOPRIVATEB2 GLOBALTOPRIVATEB##PARAMS
#define GLOBALTOPRIVATEA2D22 GLOBALTOPRIVATEA2D2##PARAMS
#define GLOBALTOPRIVATEB2D22 GLOBALTOPRIVATEB2D2##PARAMS
#define LOCALTOPRIVATEA22 LOCALTOPRIVATEA2##PARAMS
#define LOCALTOPRIVATEB22 LOCALTOPRIVATEB2##PARAMS
...
#define GLOBALTOLOCALA2 GLOBALTOLOCALA##PARAMS
#define GLOBALTOLOCALB2 GLOBALTOLOCALB##PARAMS
#define GLOBALTOPRIVATE2 GLOBALTOPRIVATE##PARAMS
#define GLOBALTOPRIVATEA2D2 GLOBALTOPRIVATEA2D##PARAMS
#define GLOBALTOPRIVATEB2D2 GLOBALTOPRIVATEB2D##PARAMS
#define LOCALTOPRIVATEA2 LOCALTOPRIVATEA##PARAMS
#define LOCALTOPRIVATEB2 LOCALTOPRIVATEB##PARAMS
#define INITACCREGISTERS2 CONCATENATE(INITACCREGISTERS,PARAMS)
#define GLOBALTOLOCALA2 CONCATENATE(GLOBALTOLOCALA,PARAMS)
#define GLOBALTOLOCALB2 CONCATENATE(GLOBALTOLOCALB,PARAMS)
#define GLOBALTOPRIVATEA2D2 CONCATENATE(GLOBALTOPRIVATEA2D,PARAMS)
#define GLOBALTOPRIVATEB2D2 CONCATENATE(GLOBALTOPRIVATEB2D,PARAMS)
#define LOCALTOPRIVATEA2 CONCATENATE(LOCALTOPRIVATEA,PARAMS)
#define LOCALTOPRIVATEB2 CONCATENATE(LOCALTOPRIVATEB,PARAMS)
#define INITACCREGISTERS2 CONCATENATE(INITACCREGISTERS,PARAMS)
#define GLOBALTOLOCALA2 CONCATENATE(GLOBALTOLOCALA,PARAMS)
#define GLOBALTOLOCALB2 CONCATENATE(GLOBALTOLOCALB,PARAMS)
#define GLOBALTOPRIVATE2 CONCATENATE(GLOBALTOPRIVATE,PARAMS)
#define GLOBALTOPRIVATEA2D2 CONCATENATE(GLOBALTOPRIVATEA2D,PARAMS)
#define GLOBALTOPRIVATEB2D2 CONCATENATE(GLOBALTOPRIVATEB2D,PARAMS)
#define LOCALTOPRIVATEA2 CONCATENATE(LOCALTOPRIVATEA,PARAMS)
#define LOCALTOPRIVATEB2 CONCATENATE(LOCALTOPRIVATEB,PARAMS)
...
INLINE_FUNC realM #define INITACCREGISTERS INITACCREGISTERS##PARAMS
#define INITACCREGISTERS INITACCREGISTERS##PARAMS
INITACCREGISTERS2() {
  realM result;
  #if VWM == 1
...
#if SA == 1
INLINE_FUNC void #define GLOBALTOLOCALA GLOBALTOLOCALA##PARAMS
#define GLOBALTOLOCALA GLOBALTOLOCALA##PARAMS
GLOBALTOLOCALA2(const __global realM* restrict agm, LOCAL_PTR realM* alm,
  ...
  for (int _mia = 0; _mia < MWA/VWM; _mia += 1) {
    ...
    for (int _kia = 0; _kia < KWA; _kia += 1) {
      ...
      int kg = _kia + la1*KWA;
      int idm = mg + GETGROUPID02() * (MWG/VWM);
      int idk = kg + kwg;
...
#if SB == 1
INLINE_FUNC void #define GLOBALTOLOCALB GLOBALTOLOCALB##PARAMS
#define GLOBALTOLOCALB GLOBALTOLOCALB##PARAMS
GLOBALTOLOCALB2(const __global realN* restrict bgm, LOCAL_PTR realN* blm,
  ...
  for (int _kib = 0; _kib < KWB; _kib += 1) {
    ...
    for (int _nib = 0; _nib < NWB/VWN; _nib += 1) {
      ...
      // Computes the indices for the global memory
      int kg = _kib + lb1*KWB;
      int idn = ng + GETGROUPID12() * (NWG/VWN);
      int idk = kg + kwg;
...
#if SA == 0 && GEMMK == 0
INLINE_FUNC realM #define GLOBALTOPRIVATEA GLOBALTOPRIVATEA##PARAMS
#define GLOBALTOPRIVATEA GLOBALTOPRIVATEA##PARAMS
GLOBALTOPRIVATEA(const __global realM* restrict agm, const int _mi,
                                   const int kSizeM, const int idk, const int kwg) {
  // Computes the indices based on strided/non-strided access
  #if STRM == 0
    int mg = _mi + GET_LOCAL_ID2(0)*(MWI/VWM);
  #elif STRM == 1
    int mg = GET_LOCAL_ID2(0) + _mi*MDIMC;
  #endif
  // Computes the indices for the global memory
  int idm = mg + GETGROUPID02() * (MWG/VWM);
...
// Same as above, but now for the B input matrix
#if SB == 0 && GEMMK == 0
INLINE_FUNC realN #define GLOBALTOPRIVATEB GLOBALTOPRIVATEB##PARAMS
#define GLOBALTOPRIVATEB GLOBALTOPRIVATEB##PARAMS
GLOBALTOPRIVATEB(const __global realN* restrict bgm, const int _ni,
                                   const int kSizeN, const int idk) {
  // Computes the indices based on strided/non-strided access
  #if STRN == 0
    int ng = _ni + GET_LOCAL_ID2(1)*(NWI/VWN);
  #elif STRN == 1
    int ng = GET_LOCAL_ID2(1) + _ni*NDIMC;
  #endif
  // Computes the indices for the global memory
  int idn = ng + GETGROUPID12() * (NWG/VWN);
...
INLINE_FUNC realN #define GLOBALTOPRIVATEA2D GLOBALTOPRIVATEA2D##PARAMS
#define GLOBALTOPRIVATEA2D GLOBALTOPRIVATEA2D##PARAMS
GLOBALTOPRIVATEA2D2(const __global real* restrict a_ptr, const int tid_y, const int _ni,
...
// Same as above, but now for the B input matrix
INLINE_FUNC realM #define GLOBALTOPRIVATEB2D GLOBALTOPRIVATEB2D##PARAMS
#define GLOBALTOPRIVATEB2D GLOBALTOPRIVATEB2D##PARAMS
GLOBALTOPRIVATEB2D2(const __global real* restrict b_ptr, const int tid_x, const int _mi,
...
#if SA == 1
INLINE_FUNC realM #define LOCALTOPRIVATEA LOCALTOPRIVATEA##PARAMS
#define LOCALTOPRIVATEA LOCALTOPRIVATEA##PARAMS
LOCALTOPRIVATEA2(LOCAL_PTR realM* alm, const int _mi, const int kg) {
  #if STRM == 0
    int mg = _mi + GET_LOCAL_ID2(0)*(MWI/VWM);
  #elif STRM == 1
    int mg = GET_LOCAL_ID2(0) + _mi*MDIMC;
...
// Same as above, but now for the B input matrix
#if SB == 1
INLINE_FUNC realN #define LOCALTOPRIVATEB LOCALTOPRIVATEB##PARAMS
#define LOCALTOPRIVATEB LOCALTOPRIVATEB##PARAMS
LOCALTOPRIVATEB2(LOCAL_PTR realN* blm, const int _ni, const int kg) {
  #if STRN == 0
    int ng = _ni + GET_LOCAL_ID2(1)*(NWI/VWN);
  #elif STRN == 1
    int ng = GET_LOCAL_ID2(1) + _ni*NDIMC;
