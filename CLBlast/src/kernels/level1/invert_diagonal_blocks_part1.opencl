...
R"(#define INVERTDIAGONALBLOCK2 INVERTDIAGONALBLOCK##PARAMS
#define TRIPLEMATMUL2 TRIPLEMATMUL##PARAMS
#define TRIPLEMATMULPART12 TRIPLEMATMULPART1##PARAMS
#define TRIPLEMATMULPART22 TRIPLEMATMULPART2##PARAMS
#define INVERTDIAGONALBLOCK22 INVERTDIAGONALBLOCK2##PARAMS
#define TRIPLEMATMUL22 TRIPLEMATMUL2##PARAMS
#define TRIPLEMATMULPART122 TRIPLEMATMULPART12##PARAMS
#define TRIPLEMATMULPART222 TRIPLEMATMULPART22##PARAMS
#define TRIPLEMATMULPART22 TRIPLEMATMULPART2##PARAMS
#define TRIPLEMATMULPART12 TRIPLEMATMULPART1##PARAMS
#define TRIPLEMATMUL2 TRIPLEMATMUL##PARAMS
#define INVERTDIAGONALBLOCK2 INVERTDIAGONALBLOCK##PARAMS
...
#define TRIPLEMATMULPART12 TRIPLEMATMULPART1##PARAMS
#define TRIPLEMATMULPART22 TRIPLEMATMULPART2##PARAMS
#define INVERTDIAGONALBLOCK2 CONCATENATE(INVERTDIAGONALBLOCK,PARAMS)
#define TRIPLEMATMUL2 CONCATENATE(TRIPLEMATMUL,PARAMS)
#define TRIPLEMATMULPART12 CONCATENATE(TRIPLEMATMULPART1,PARAMS)
#define TRIPLEMATMULPART22 CONCATENATE(TRIPLEMATMULPART2,PARAMS)
#define INVERTDIAGONALBLOCK2 CONCATENATE(INVERTDIAGONALBLOCK,PARAMS)
#define TRIPLEMATMUL2 CONCATENATE(TRIPLEMATMUL,PARAMS)
#define TRIPLEMATMULPART12 CONCATENATE(TRIPLEMATMULPART1,PARAMS)
#define TRIPLEMATMULPART22 CONCATENATE(TRIPLEMATMULPART2,PARAMS)
...
#endif
void #define INVERTDIAGONALBLOCK INVERTDIAGONALBLOCK##PARAMS
#define INVERTDIAGONALBLOCK INVERTDIAGONALBLOCK##PARAMS
INVERTDIAGONALBLOCK2(const int n, __global const real* restrict src, const int src_offset, const int src_ld,
                         __global real* restrict dest, const int outer_block_size,
                         const int unit_diagonal, const int is_upper)
{
  const int thread_index = GET_LOCAL_ID2(0);
  const int block_index = GET_GROUP_ID2(0);
...
INLINE_FUNC void #define TRIPLEMATMUL TRIPLEMATMUL##PARAMS
#define TRIPLEMATMUL TRIPLEMATMUL##PARAMS
TRIPLEMATMUL2(const int size, const bool upper, const int part, LOCAL_PTR real* blm, int n,
  ...
  const int by   = GET_GROUP_ID2(1) / num_pages;
  const int page = GET_GROUP_ID2(1) % num_pages;
  const int lidx = GET_LOCAL_ID2(0);
  const int lidy = GET_LOCAL_ID2(1);
  const int ibx  = GET_GROUP_ID2(0) * (GET_LOCAL_SIZE2(0) * TMMWGSY);
  const int iby  = by*16;
  const int id   = lidx + lidy*GET_LOCAL_SIZE2(0);
  ...
  for (int k = 0; k < current_size; k += 16) {
    ...
    for (int i = 0; i < 16; i += (size/4) ) {  // += GET_LOCAL_SIZE2(0)
      for (int _j = 0; _j < 16; _j += TMMWGSY ) {  // += GET_LOCAL_SIZE2(1)
...
INLINE_FUNC void #define TRIPLEMATMULPART1 TRIPLEMATMULPART1##PARAMS
#define TRIPLEMATMULPART1 TRIPLEMATMULPART1##PARAMS
TRIPLEMATMULPART12(const int size, const bool upper, LOCAL_PTR real* blm, int n,
  ...
  TRIPLEMATMUL2(size, upper, 1, blm, n, agm, bgm, cgm, lda, ldb, ldc, current_size, num_pages, block_size);
}
// Triple matrix-multiplication kernel part 1: B12 = -B11 * B12 (upper) or B21 = -B22 * B21 (lower)
INLINE_FUNC void #define TRIPLEMATMULPART2 TRIPLEMATMULPART2##PARAMS
#define TRIPLEMATMULPART2 TRIPLEMATMULPART2##PARAMS
TRIPLEMATMULPART22(const int size, const bool upper, LOCAL_PTR real* blm, const int n,
                                   __global real* dest, int current_size, int num_pages, const int block_size) {
  // Emulates a 3D grid: NX * (NY * num_pages)
  const int page = GET_GROUP_ID2(1) % num_pages;
  // Computes the destination block offset:
  ...
  const int ldb = block_size;
  const int ldc = block_size;
  TRIPLEMATMUL2(size, upper, 2, blm, n, agm, bgm, cgm, lda, ldb, ldc, current_size, num_pages, block_size);
}
#endif
// =================================================================================================
