...
Xgemm<T>::Xgemm(Queue &queue, EventPointer event, const std::string &name):
    Routine(queue, event, name,
            ...
            PrecisionValue<T>(), {}, std::unordered_map<std::string, std::vector<const char *>>{ 
                {"default",
                  {
...
void Xgemm<T>::DoGemm(const Layout layout,
  ...
  log_debug("do gemm");
  const auto input = database::Input{m,n,k};
  // Two methods to choose from, select which one to run
  const auto do_gemm_direct = UseDirectKernel(m, n, k, db_.GetParameterValue(input, "XGEMM_MIN_INDIRECT_SIZE", "GemmRoutine"));
  log_debug(ToString(db_.GetParameterValue(input, "XGEMM_MIN_INDIRECT_SIZE", "GemmRoutine")));
  const auto gemm_kernel_id = (do_gemm_direct) ? 0 : db_.GetParameterValue(input, "GEMMK", "Xgemm");
...
void Xgemm<T>::GemmIndirect(const size_t m, const size_t n, const size_t k,
  ...
  // retrieve the input
  auto input = database::Input{m, n, k};

  log_debug("entering gemm indirect");

  // retrieve the databases with parameters
  Database db_indirect = db_("Xgemm");

  // Calculates the ceiled versions of m, n, and k
  const auto m_ceiled = Ceil(m, db_indirect.GetParameterValue(input, "MWG"));
  const auto n_ceiled = Ceil(n, db_indirect.GetParameterValue(input, "NWG"));
  const auto k_ceiled = Ceil(k, db_indirect.GetParameterValue(input, "KWG") * db_indirect.GetParameterValue(input, "KREG"));

  // Computes the first and second "internal" (ceiled) dimensions of the 3 matrices taking into account
  // whether the matrices need to be rotated or not for the kernel.
  size_t a_one_i, a_two_i, b_one_i, b_two_i, c_one_i, c_two_i;
  CalculateInternalDimensions(m, n, k, db_indirect.GetParameterValue(input, "MWG"), db_indirect.GetParameterValue(input, "NWG"), db_indirect.GetParameterValue(input, "KWG") * db_indirect.GetParameterValue(input, "KREG"),
                              a_one_i, a_two_i, b_one_i, b_two_i, c_one_i, c_two_i,
                              db_indirect.GetParameterValue(input, "GEMMK"));
  ...
  if (!IsMultiple(b_temp_offset, db_indirect.GetParameterValue(input, "VWN"))) { throw BLASError(StatusCode::kUnexpectedError); }
  if (!IsMultiple(c_temp_offset, db_indirect.GetParameterValue(input, "VWM"))) { throw BLASError(StatusCode::kUnexpectedError); }
  ...
  auto values = db_indirect.GetValuesStringForInput(input);

  log_debug("running gemm kernel");

  // Retrieves the Xgemm kernel from the compiled binary
  auto kernel = Kernel(program_, "Xgemm" + values);
  ...
  kernel.SetArgument(8, static_cast<int>(b_temp_offset / db_indirect.GetParameterValue(input, "VWN")));
  kernel.SetArgument(9, static_cast<int>(c_temp_offset / db_indirect.GetParameterValue(input, "VWM")));

  // Computes the global and local thread sizes
  const auto global_divider_one = c_want_rotated_(db_indirect.GetParameterValue(input, "GEMMK")) ? db_indirect.GetParameterValue(input, "NWG") : db_indirect.GetParameterValue(input, "MWG");
  const auto global_divider_two = c_want_rotated_(db_indirect.GetParameterValue(input, "GEMMK")) ? db_indirect.GetParameterValue(input, "MWG") : db_indirect.GetParameterValue(input, "NWG");
  const auto global = std::vector<size_t>{
    (c_one_i * db_indirect.GetParameterValue(input, "MDIMC")) / global_divider_one,
    (c_two_i * db_indirect.GetParameterValue(input, "NDIMC")) / global_divider_two
  };
  const auto local = std::vector<size_t>{db_indirect.GetParameterValue(input, "MDIMC"), db_indirect.GetParameterValue(input, "NDIMC")};
...
void Xgemm<T>::GemmDirect(const size_t m, const size_t n, const size_t k,
  ...
  log_debug("entering gemm direct");

  // Retrieves the proper XgemmDirect kernel name 
  const auto name = (a_do_transpose) ? (b_do_transpose ? "XgemmDirectTT" : "XgemmDirectTN") :
                                       (b_do_transpose ? "XgemmDirectNT" : "XgemmDirectNN");

  // retrieve the databases with parameters
  Database db_direct = db_("XgemmDirect");

  // retrieve parameters string
  auto values = db_direct.GetValuesStringForInput(input);

  log_debug("running gemm kernel: ");
  log_debug(name + values);

  // Retrieves the proper XgemmDirect kernel from the compiled binary
  auto kernel = Kernel(program_, name + values);
  ...
  // Computes the global and local thread sizes
  const auto wgd = db_direct.GetParameterValue(input, "WGD");
  const auto m_ceiled = Ceil(m, wgd); // 16
  const auto n_ceiled = Ceil(n, wgd); // 16

  log_debug("wgd: " + ToString(wgd));
  log_debug("m/n: " + ToString(m) + " " + ToString(n));
  log_debug("m/n ceil: " + ToString(Ceil(m, wgd)) + " " + ToString(Ceil(n, wgd)));

  const auto global = std::vector<size_t>{
  //  CeilDiv(m * db_direct.GetParameterValue(input, "MDIMCD"), db_direct.GetParameterValue(input, "WGD")),
  //  CeilDiv(n * db_direct.GetParameterValue(input, "NDIMCD"), db_direct.GetParameterValue(input, "WGD"))
      (m_ceiled * db_direct.GetParameterValue(input, "MDIMCD")) / wgd, // 16 * 8 / 16
      (n_ceiled * db_direct.GetParameterValue(input, "NDIMCD")) / wgd // 16 * 8 / 16
  };
  const auto local = std::vector<size_t>{db_direct.GetParameterValue(input, "MDIMCD"), db_direct.GetParameterValue(input, "NDIMCD")};
  
  log_debug("global: " + ToString(global[0]) + " " + ToString(global[1]));
  log_debug("MDIMCD: " + ToString(db_direct.GetParameterValue(input, "MDIMCD")) + " " + ToString(db_direct.GetParameterValue(input, "NDIMCD")));
  log_debug("local: " + ToString(local[0]) + " " + ToString(local[1]));
